...
3. Update Lambda to use image:
   aws lambda update-function-code --function-name naverplace_send_inform \
...
aws lambda invoke --function-name naverplace_send_inform \
...
aws logs filter-log-events --log-group-name /aws/lambda/naverplace_send_inform \
...
# Check logs
aws logs tail /aws/lambda/naverplace_send_inform --follow | grep "SENS"
```
