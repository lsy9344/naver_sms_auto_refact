...
**Status:** Ready for Review *(QA fixes applied and verified)*
...
- **Blocking Issues (2025-10-28):**
  ...
  2. `tests/integration/test_rules_regression.py:883` fails: the config under `src/config/rules.yaml:110-147` still uses the legacy `holiday_event_digest` template and lacks channel/template params, so the Slack-enabled regression never matches the new expectations in `tests/fixtures/legacy_expected_actions.json#L123-L140`.
