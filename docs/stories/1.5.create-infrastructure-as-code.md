...
- [ ] **Establish IaC tool choice and bootstrap environment** (AC: 1, 3, 4, 9)
  ...
  - [ ] Initialize Terraform root module with AWS provider v5.0+
  - [ ] Configure backend.tf for remote state (S3 bucket naming strategy: `terraform-state-{account-id}`)
  - [ ] Create `scripts/deploy_infra.sh` bootstrap script with version checks and workspace selection
  - [ ] Document IAM permissions required in `docs/qa/required-iam-policy.json` (attach to README)

### Phase 2: Infrastructure Encoding

- [ ] **Encode ECR Repository module** (AC: 1, 2, 7, 8)
  - [ ] Create `modules/ecr/main.tf` provisioning ECR repo with lifecycle policy (reference Stories 1.1 output)
  - [ ] Define `modules/ecr/variables.tf` (environment, repository_name, image_tag_mutability)
  - [ ] Define `modules/ecr/outputs.tf` (repository_uri, registry_id)
  - [ ] Add environment-specific overrides to `environments/{dev,staging,prod}.tfvars`

- [ ] **Encode Secrets Manager module** (AC: 1, 2, 6, 7, 8)
  - [ ] Create `modules/secrets-manager/main.tf` provisioning three secrets:
    - `naver-sms-automation/naver-credentials` (sensitive)
    - `naver-sms-automation/sens-credentials` (sensitive)
    - `naver-sms-automation/telegram-credentials` (sensitive)
  - [ ] Use `sensitive()` wrapper for secret values; placeholder values in `.tfvars`
  - [ ] Define `modules/secrets-manager/variables.tf` with secret_value as sensitive input
  - [ ] Define `modules/secrets-manager/outputs.tf` (secret ARNs, names)
  - [ ] Create `scripts/setup-secrets.sh` to populate actual values from external source (documented)

- [ ] **Encode CloudWatch Logging & Alarms module** (AC: 1, 2, 7, 8)
  - [ ] Create `modules/cloudwatch/main.tf` provisioning:
  ...
  - [ ] Define `modules/cloudwatch/variables.tf` (retention_days, alarm_thresholds)
  - [ ] Define `modules/cloudwatch/outputs.tf` (log_group_name, dashboard_url, alarm_arns)

### Phase 3: Security & Documentation

- [ ] **Secure secrets handling and create security documentation** (AC: 3, 4, 6, 11)
  - [ ] Create `.gitignore` entries for `*.tfvars` (except environment examples)
  - [ ] Create `infrastructure/terraform/example.tfvars` showing placeholder structure
  ...
  - [ ] Create least-privilege IAM policy for CI/CD in `docs/qa/required-iam-policy.json`

- [ ] **Create operational documentation** (AC: 4, 9)
  - [ ] Write `infrastructure/README.md` with all sections (see AC 4 requirements)
  - [ ] Create troubleshooting guide in `infrastructure/TROUBLESHOOTING.md`
  - [ ] Add "Local Development Setup" section with `.env.local` template
  - [ ] Add "State Recovery" section (emergency rollback procedures)

- [ ] **Document migration plan from ad-hoc to IaC** (AC: 8)
  - [ ] Create `docs/migration-plan-1.5.md` specifying:
...
- [ ] **Establish CI pipeline for IaC validation** (AC: 5, 10, 11)
...
**Agent Model Used:** _[To be filled by development agent during implementation]_

**Debug Log References:** _[To be filled by development agent if issues encountered]_

**Completion Notes List:** _[To be filled by development agent with implementation details, any deviations from plan, gotchas]_

**File List:** _[To be filled by development agent with all created/modified files during story implementation]_
