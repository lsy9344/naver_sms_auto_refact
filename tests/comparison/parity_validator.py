...
class ParityValidator:
    ...
    def execute_legacy_handler(
        ...
        mock_services: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            import sys
            original_code_path = (
                Path(__file__).resolve().parents[2] / "original_code"
            )
            ...
            if user_data:  # Only if not empty response
                results = reservation_check(user_data)
                outputs["actions"].extend(
                    [{"action_type": "reservation_check", "success": True}]
                )

                # Execute option check
                option_results = option_sms_check(user_data)
                if option_results:
                    outputs["actions"].extend(
    ...
    def validate_idempotency(
        ...
        try:
            ...
            output1 = self.execute_refactored_handler(scenario_context, mock_services1)
            ...
            output2 = self.execute_refactored_handler(scenario_context_run2, mock_services2)
            sms_count_2 = len(mock_services2.get("sms_calls", []))

            if sms_count_2 > 0:
                return False, f"Not idempotent: SMS sent on both runs ({sms_count_1} vs {sms_count_2})"
